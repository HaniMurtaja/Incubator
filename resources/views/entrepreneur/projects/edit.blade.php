...
<form method="post" action="{{ route('entrepreneur.projects.update', $project) }}" enctype="multipart/form-data" class="ep-form-shell">
    @csrf
    @method('put')
    @include('entrepreneur.projects.partials.form')
    <button type="submit" class="ep-form-submit">{{ $isAr ? 'تحديث' : 'Update' }}</button>
</form>
@endsection
