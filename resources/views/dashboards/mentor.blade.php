    ...
    <div class="col-md-6">
        <div class="dash-stat-card">
            ...
            <div class="dash-stat-label">
                {{ $isAr ? 'المشاريع المسندة' : 'Assigned Projects' }}
            </div>
            <div class="dash-stat-value">{{ $stats['assigned_projects'] }}</div>
        </div>
    </div>
    <div class="col-md-6">
        <div class="dash-stat-card">
            ...
            <div class="dash-stat-label">
                {{ $isAr ? 'التسليمات بانتظار التقييم' : 'Pending Reviews' }}
            </div>
            <div class="dash-stat-value">{{ $stats['pending_submissions'] }}</div>
        </div>
    </div>
</div>
...
<div class="dash-nav-tabs mb-4">
    <a href="{{ route('mentor.command.index') }}"
       class="dash-nav-tab {{ request()->routeIs('mentor.command.*') ? 'active' : '' }}">
        {{ $isAr ? 'مركز قيادة الموجه' : 'Command Center' }}
    </a>
    <a href="{{ route('mentor.calendar.index') }}"
       class="dash-nav-tab {{ request()->routeIs('mentor.calendar.*') ? 'active' : '' }}">
        {{ $isAr ? 'تقويم الإرشاد' : 'Mentorship Calendar' }}
    </a>
    <a href="{{ route('mentor.rounds.index') }}"
       class="dash-nav-tab {{ request()->routeIs('mentor.rounds.*') ? 'active' : '' }}">
        {{ $isAr ? 'جولات الاحتضان' : 'Incubator Rounds' }}
    </a>
    <a href="{{ route('mentor.projects.index') }}"
       class="dash-nav-tab {{ request()->routeIs('mentor.projects.*') ? 'active' : '' }}">
        {{ $isAr ? 'المشاريع' : 'Projects' }}
    </a>
    <a href="{{ route('mentor.tasks.index') }}"
       class="dash-nav-tab {{ request()->routeIs('mentor.tasks.*') ? 'active' : '' }}">
        {{ $isAr ? 'المهام' : 'Tasks' }}
    </a>
</div>
...
<div class="row g-3">
    <div class="col-md-6">
        <div class="dash-action-card">
            ...
            <div class="dash-action-desc">
                {{ $isAr ? 'عرض المشاريع المتاحة لك ومتابعة مراحل كل مشروع.' : 'Track your assigned projects and stage progress.' }}
            </div>
            <a href="{{ route('mentor.projects.index') }}" class="dash-action-btn blue">
    ...
    <div class="col-md-6">
        <div class="dash-action-card">
            ...
            <div class="dash-action-title">
                {{ $isAr ? 'إدارة المهام والتقييم' : 'Tasks & Evaluation' }}
            </div>
            <div class="dash-action-desc">
                {{ $isAr ? 'إنشاء المهام، مراجعة التسليمات، وإرسال الملاحظات.' : 'Create tasks, review submissions, and provide feedback.' }}
            </div>
            <a href="{{ route('mentor.tasks.index') }}" class="dash-action-btn green">
                {{ $isAr ? 'الذهاب للمهام' : 'Open Tasks' }}
            </a>
        </div>
    </div>
</div>
